...
import httplib2
import re, io, os, tempfile, datetime, base64, cPickle
...
def upload_from_drive(stream, drive_id, request):
	...
	try:
		...
		fields = {
			...
			'date': meta['/CreationDate'][2:6] + '-' + meta['/CreationDate'][6:8] + '-' + meta['/CreationDate'][8:10] if '/CreationDate' in meta else '2000-01-01',
			'owner': owner,
			'drive_id': drive_id,
			'type': 1,
		}
...
def get_credentials(request):
	if request.user.is_authenticated():
		...
		response = {'error': True, 'message':'Debes anadir un enlace de Google Drive a la solicitud.'}
